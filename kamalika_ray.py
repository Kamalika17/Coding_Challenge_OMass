...
input_file = sys.argv[1] # Requires input as command-line argument
...
gene_list = df['target.approvedSymbol'].values.tolist()
disease_list = df['disease.name'].values.tolist()
nx.draw_networkx_nodes(G, pos=pos, nodelist=gene_list,
                       node_color='red', label='Genes', node_size=5)
nx.draw_networkx_nodes(G, pos=pos, nodelist=disease_list,
                       node_color='blue', label='Diseases', node_size=5)
nx.draw_networkx_edges(G, pos=pos, edge_color='darkgrey', label='Associations')
plt.legend()
