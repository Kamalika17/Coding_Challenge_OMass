...
number="$(awk '{print $--NF}' $inputfile | grep "GPCR" | wc -l)"
echo 'Number of the GPCRs:' $number
...
rec="$(grep "SLC15A4" $inputfile | sed 's/NA/0/g' | tee 'output/gene_disease_opt_new.csv' | wc -l)"
echo 'Number of the records for the gene SLC15A4:' $rec